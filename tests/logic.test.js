...
vm.runInContext(`
  ...
  const DQ_MISSING_THRESHOLD_PCT = 5;
  ${extractFn('computeDataQuality')}
  ${extractFn('buildSampleRows')}
`, sandbox);
...
console.log('computeDataQuality');
test('counts exact-duplicate rows once (the 2nd+ occurrence)', () => {
  const rows = [{a:1,b:'x'}, {a:1,b:'x'}, {a:2,b:'y'}];
  const {duplicateCount} = sandbox.computeDataQuality(rows, [{name:'a'},{name:'b'}]);
  assert.strictEqual(duplicateCount, 1);
});
test('flags a column above the missing-data threshold, with the correct percentage', () => {
  const rows = [
    {a:1,b:null},{a:2,b:''},{a:3,b:'x'},{a:4,b:'y'},{a:5,b:'z'},
    {a:6,b:'w'},{a:7,b:'v'},{a:8,b:'u'},{a:9,b:'t'},{a:10,b:'s'}
  ]; // b is 20% missing
  const {columnsWithMissing} = sandbox.computeDataQuality(rows, [{name:'a'},{name:'b'}]);
  assert.strictEqual(columnsWithMissing.length, 1);
  assert.strictEqual(columnsWithMissing[0].name, 'b');
  assert(Math.abs(columnsWithMissing[0].missingPct - 20) < 1e-9);
});
test('does not flag a column below the missing-data threshold', () => {
  const rows = Array.from({length:100}, (_,i) => ({a: i===0 ? null : i})); // 1% missing
  const {columnsWithMissing} = sandbox.computeDataQuality(rows, [{name:'a'}]);
  assert.strictEqual(columnsWithMissing.length, 0);
});

console.log('buildSampleRows (sample-data button)');
test('produces 140 rows with the expected columns', () => {
  const rows = sandbox.buildSampleRows();
  assert.strictEqual(rows.length, 140);
  assert.deepStrictEqual(Object.keys(rows[0]), ['Date','Region','Sales Rep','Product Category','Target','Revenue','Units Sold']);
});
test('is deterministic — same output on repeated calls', () => {
  assert.deepStrictEqual(sandbox.buildSampleRows(), sandbox.buildSampleRows());
});
test('dates are ascending, all numeric fields are finite and non-negative', () => {
  const rows = sandbox.buildSampleRows();
  for(let i=1; i<rows.length; i++) assert(rows[i].Date >= rows[i-1].Date, 'dates out of order at row '+i);
  rows.forEach((r,i) => {
    assert(Number.isFinite(r.Target) && r.Target > 0, 'bad Target at row '+i);
    assert(Number.isFinite(r.Revenue) && r.Revenue >= 0, 'bad Revenue at row '+i);
    assert(Number.isFinite(r['Units Sold']) && r['Units Sold'] >= 0, 'bad Units Sold at row '+i);
  });
});
test('Target/Revenue/Units are NOT misdetected as ID columns (regression check)', () => {
  // Caught during development: near-unique numeric values across all rows tripped inferColumns'
  // ID heuristic (>92% unique looks like a serial number), which would have silently excluded
  // these columns from every chart and KPI in the sample dashboard.
  const rows = sandbox.buildSampleRows();
  const cols = sandbox.inferColumns(rows);
  ['Target','Revenue','Units Sold'].forEach(name => {
    const c = cols.find(x => x.name === name);
    assert.strictEqual(c.type, 'numeric', name+' should infer as numeric');
    assert.strictEqual(c.isId, false, name+' should NOT be flagged as an ID column');
  });
  ['Region','Sales Rep','Product Category'].forEach(name => {
    assert.strictEqual(cols.find(x => x.name === name).type, 'categorical', name+' should infer as categorical');
  });
  assert.strictEqual(cols.find(x => x.name === 'Date').type, 'date');
});
